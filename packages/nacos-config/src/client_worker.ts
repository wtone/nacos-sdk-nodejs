 ...
 */
import { API_ROUTE, ClientOptionKeys, NacosHttpError, IClientWorker, IConfiguration, ISnapshot, ACCESS_TOKEN } from "./interface";
...
export class ClientWorker extends Base implements IClientWorker {
	...
	private async accessToken(): Promise<String> {
		if (!this.LOGIN_OPTIONS?.username || !this.LOGIN_OPTIONS?.password) {
			return "";
		}

		const access_token_tmp_path = __dirname + "/access_token.json";
		...
		if (fs.existsSync(access_token_tmp_path)) {
			this.AccessToken = require(access_token_tmp_path);
			if (this.AccessToken && this.AccessToken.expiresAt < now) {
				return this.AccessToken.accessToken;
			}
		}

		//请求login接口 获取access_token
		let content = await this.httpAgent.request(this.LOGIN, {
			method: "POST",
			data: this.LOGIN_OPTIONS,
		});

		this.AccessToken = JSON.parse(content);
		this.AccessToken.expiresAt = now + this.AccessToken.tokenTtl * 1000;

		fs.writeFileSync(access_token_tmp_path, JSON.stringify(this.AccessToken));
	...
	private async checkServerConfigInfo() {
		...
		if (tenant) {
			Object.assign(postData, { tenant, accessToken: await this.accessToken() });
		}
